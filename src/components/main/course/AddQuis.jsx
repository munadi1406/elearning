  ...
  const handleCreate = () => {
    ...
    for (let index = 0; index < jumlahSoal; index++) {
      ...
      for (let i = 0; i < jumlahOpsiJawaban; i++) {
        const jawabanInput = (
          <textarea key={`jawaban${i}`} className={style.jawaban} />
        );

        const radioInput = (
          <input key={`radio${i}`} type="radio" name={`soal-${index}`} />
        );

        const labelJawaban = (
          <label key={`label${i}`} className={style.label}>{`Jawaban ${
            i + 1
          }`}</label>
        );
          ...
          <div className="grid grid-cols-2 gap-2">
            ...
            <div className="w-full">
              ...
              <input
                ...
                id="namaTugas"
                className={`${style.input}`}
                placeholder="Jumlah Opsi Jawaban"
